export async function onRequest(context) {
  ...
  // 1. Decode the URL path to handle folders with spaces or special characters
  let path = decodeURIComponent(url.pathname);
  if (path.startsWith('/')) path = path.slice(1);
  if (path.endsWith('/')) path = path.slice(0, -1);

  // 2. PASS-THROUGH: If the request is for an image file, stream it directly from R2
  if (path.match(/\.(jpg|jpeg|png|webp|gif)$/i)) {
    ...
    headers.set('Cache-Control', 'public, max-age=604800'); // Cache images for 7 days
    return new Response(object.body, { headers });
  }

  // 3. LIVE SCAN: Get all "folders" (prefixes) in the root of the R2 bucket
  const rootList = await env.PHOTOS_BUCKET.list({ delimiter: '/' });
  const folders = rootList.delimitedPrefixes.map(p => p.slice(0, -1)); // Remove trailing slash

  // Determine which folder gallery the user is currently viewing
  // Default to the first folder alphabetically if they are on the homepage
  const activeFolder = path || folders[0] || '';

  // 4. LIVE SCAN: Get all images inside the active folder
  let images = [];
  if (activeFolder) {
  ...
  // 7. RETURN THE FULL HTML PAGE WITH CLEAN STYLES
  const html = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
      ...
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .grid-item {
          ...
          overflow: hidden;
          background: #fafafa;
          aspect-ratio: 3 / 2; /* Forces a clean layout layout. Adjust or remove if you want natural heights */
        }
        .grid-item img {
          width: 100%;
          height: 100%;
